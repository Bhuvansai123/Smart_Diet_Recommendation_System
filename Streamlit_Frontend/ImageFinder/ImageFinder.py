import requests
from bs4 import BeautifulSoup
from urllib.parse import quote_plus

NOT_FOUND_LINK = 'data:image/png;base64,'

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36"
}


def get_images_links(search_term: str) -> str:
    try:
        search_url = f"https://www.google.com/search?q={quote_plus(search_term)}&site=webhp&tbm=isch"
        response = requests.get(search_url, headers=HEADERS, timeout=10)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        for img in soup.find_all('img'):
            src = img.get('src', '')
            if src.startswith('http'):
                return src
    except Exception:
        pass

    # Reliable fallback image source
    return f"https://source.unsplash.com/400x300/?{quote_plus(search_term + ' food')}"
